...
const SHIFT_COLORS: Record<string, string> = {
  MORNING:        'bg-amber-500/15 text-amber-700 border-amber-500/20',
  AFTERNOON:      'bg-orange-500/15 text-orange-700 border-orange-500/20',
  DAY:            'bg-sky-500/15 text-sky-700 border-sky-500/20',
  PM_SHIFT:       'bg-purple-500/15 text-purple-700 border-purple-500/20',
  NIGHT:          'bg-indigo-500/15 text-indigo-700 border-indigo-500/20',
  LATE:           'bg-rose-500/15 text-rose-700 border-rose-500/20',
  ON_CALL:        'bg-teal-500/15 text-teal-700 border-teal-500/20',
  OFF:            'bg-foreground/5 text-foreground/45 border-foreground/10',
  ANNUAL_LEAVE:   'bg-green-500/15 text-green-700 border-green-500/20',
  MATERNITY_LEAVE:'bg-pink-500/15 text-pink-700 border-pink-500/20',
  SICK_LEAVE:     'bg-red-500/15 text-red-600 border-red-500/20',
}
...
const STATUS_BADGE: Record<RowStatus, string> = {
  on_time:   'bg-green-500/10 text-green-600 border-green-500/20',
  ...
  no_record: 'bg-foreground/5 text-foreground/45 border-foreground/10',
    ...
    <div className="p-6 max-w-[1400px] mx-auto space-y-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-foreground/50 mt-0.5">Roster vs actual attendance · {date}</p>
        </div>
        <input
          type="date"
          value={date}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        {([
          { key: 'on_time',   label: 'On Time',   color: 'text-green-600',  bg: 'bg-green-500/10',  border: 'border-green-500/20'  },
          { key: 'late',      label: 'Late',       color: 'text-amber-700',  bg: 'bg-amber-500/10',  border: 'border-amber-500/20'  },
          { key: 'no_show',   label: 'No Show',    color: 'text-red-600',    bg: 'bg-red-500/10',    border: 'border-red-500/20'    },
          { key: 'partial',   label: 'Partial',    color: 'text-orange-700', bg: 'bg-orange-500/10', border: 'border-orange-500/20' },
          { key: 'on_leave',  label: 'On Leave',   color: 'text-blue-600',   bg: 'bg-blue-500/10',   border: 'border-blue-500/20'   },
          { key: 'off',       label: 'Off',        color: 'text-foreground/40', bg: 'bg-white/60',    border: 'border-foreground/10' },
          ...
          <button
            ...
            className={`rounded-xl border ${s.border} ${s.bg} p-3 text-left transition-all hover:brightness-105 ${statusFilter === s.key ? 'ring-1 ring-foreground/20' : ''}`}
          >
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        {statusFilter !== 'all' && (
          <button
            ...
            className="px-3 py-2 rounded-lg text-xs text-foreground/60 border border-foreground/15 hover:text-foreground hover:bg-white transition-colors"
          >
            Clear filter ✕
          </button>
        )}
      ...
      <p className="text-xs text-foreground/40">{filtered.length} employees</p>

      {/* Table */}
      {loading ? (
        <div className="text-center py-16 text-foreground/40 text-sm">Loading…</div>
      ) : (
        <div className="rounded-xl border border-foreground/10 bg-white/60 overflow-hidden">
          {/* Table header */}
          <div className="grid grid-cols-[1fr_140px_120px_120px_80px_100px] gap-x-4 px-4 py-2.5 bg-white border-b border-foreground/10 text-[10px] font-semibold uppercase tracking-wider text-foreground/50">
            <span>Employee</span>
          ...
          <div className="divide-y divide-foreground/6">
            {filtered.length === 0 ? (
              <div className="text-center py-12 text-foreground/40 text-sm">No employees match your filters</div>
            ) : filtered.map(emp => {
              ...
              return (
                <div
                  key={emp.id}
                  className="grid grid-cols-[1fr_140px_120px_120px_80px_100px] gap-x-4 px-4 py-3 hover:bg-white transition-colors items-center"
                >
                  {/* Name */}
                  <div className="flex items-center gap-2.5 min-w-0">
                    <div className="h-8 w-8 rounded-full bg-foreground/8 flex items-center justify-center text-xs font-bold text-foreground/60 flex-shrink-0">
                      {emp.name.split(' ').map(w => w[0]).slice(0, 2).join('')}
                    </div>
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-foreground truncate">{emp.name}</p>
                      <p className="text-xs text-foreground/50 truncate">{emp.staffId} · {dept?.code ?? emp.departmentId.slice(0, 3)}</p>
                    </div>
                  </div>
                  ...
                  <div>
                    {slot ? (
                      <div>
                        <span className={`inline-block px-2 py-0.5 rounded border text-[10px] font-semibold ${SHIFT_COLORS[slot.shiftType] ?? 'bg-foreground/5 text-foreground/40 border-foreground/10'}`}>
                          {SHIFT_LABEL[slot.shiftType] ?? slot.shiftType}
                        </span>
                        {slot.startTime && (
                          <p className="text-[10px] text-foreground/40 mt-0.5">{fmt12(slot.startTime)} – {fmt12(slot.endTime)}</p>
                        )}
                      </div>
                    ) : (
                      <span className="text-xs text-foreground/30">Not rostered</span>
                    )}
                  </div>

                  {/* Clock in */}
                  <div>
                    <p className={`text-sm font-mono font-semibold ${rec?.clockIn ? 'text-green-600' : 'text-foreground/30'}`}>
                      {rec ? fmtTime(rec.clockIn) : '—'}
                    </p>
                    {rec?.lateMinutes && rec.lateMinutes > 0 ? (
                      <p className="text-[10px] text-amber-700/70">{rec.lateMinutes}m late</p>
                    ) : null}
                  </div>

                  {/* Clock out */}
                  <div>
                    <p className={`text-sm font-mono font-semibold ${rec?.clockOut ? 'text-foreground/60' : rec?.clockIn ? 'text-red-600/60' : 'text-foreground/30'}`}>
                  ...
                  {/* Hours */}
                  <div>
                    <p className={`text-sm font-semibold ${hours > 9 ? 'text-violet-700' : hours > 0 ? 'text-foreground/60' : 'text-foreground/30'}`}>
                      {hours > 0 ? `${hours.toFixed(1)}h` : '—'}
                    </p>
                    {slot?.plannedHours && hours > 0 ? (
                      <p className="text-[10px] text-foreground/35">of {slot.plannedHours}h</p>
                    ) : null}
                  </div>

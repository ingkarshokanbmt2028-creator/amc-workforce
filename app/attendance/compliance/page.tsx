      ...
      <table className="w-full text-xs">
        <thead>
          <tr className="bg-foreground/[0.03] border-b border-foreground/10">
            ...
            <th className="px-3 py-2 text-left font-semibold text-foreground/50">Clock In</th>
            <th className="px-3 py-2 text-left font-semibold text-foreground/50">Clock Out</th>
            <th className="px-3 py-2 text-right font-semibold text-foreground/50">Hrs</th>
            <th className="px-3 py-2 text-right font-semibold text-foreground/50">Late</th>
            <th className="px-3 py-2 text-left font-semibold text-foreground/50">Status</th>
          </tr>
        </thead>
        <tbody>
          {sortedRows.map((r, i) => {
            ...
            return (
              <tr
                key={`${r.date}-${i}`}
                className={`border-t border-foreground/6 ${isOff ? 'opacity-40' : ''} ${
                  r.compliance === 'NO_SHOW' ? 'bg-red-500/[0.04]' :
                  r.compliance === 'LATE'    ? 'bg-amber-500/[0.04]' : ''
                }`}
              >
                <td className="px-4 py-2 font-medium text-foreground/70">{fmtDate(r.date)}</td>
                <td className="px-3 py-2">
                  {r.rosterShift ? (
                    <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-foreground/8 text-foreground/60">
                      {r.rosterShift.replace(/_/g, ' ')}
                    </span>
                  ) : <span className="text-foreground/30">—</span>}
                </td>
                <td className="px-3 py-2 font-mono text-foreground/55">
                  {r.scheduledStart && r.scheduledEnd
                    ? `${r.scheduledStart}–${r.scheduledEnd}`
                    : '—'}
                </td>
                <td className={`px-3 py-2 font-mono ${r.clockIn ? (r.lateMinutes > 0 ? 'text-amber-600' : 'text-green-600') : 'text-red-600/60'}`}>
                  {fmtTime(r.clockIn)}
                </td>
                <td className={`px-3 py-2 font-mono ${r.clockOut ? 'text-foreground/55' : r.clockIn ? 'text-amber-600/60' : 'text-foreground/30'}`}>
                  {fmtTime(r.clockOut)}
                </td>
                <td className="px-3 py-2 text-right text-foreground/55">
                  {r.actualHours > 0 ? `${r.actualHours.toFixed(1)}h` : '—'}
                </td>
                <td className={`px-3 py-2 text-right ${r.lateMinutes > 0 ? 'text-amber-600 font-semibold' : 'text-foreground/30'}`}>
...
function EmployeeCard({ summary, expanded, onToggle }: {
  ...
  const rateColor = summary.complianceRate >= 90 ? 'text-green-600' : summary.complianceRate >= 75 ? 'text-amber-600' : 'text-red-600'
  ...
  return (
    <div className={`rounded-xl border transition-all ${
      ...
      >
        {/* Avatar */}
        <div className="w-9 h-9 rounded-full bg-foreground/8 flex items-center justify-center text-xs font-bold text-foreground/60 flex-shrink-0">
          {summary.employeeName.split(' ').map(w => w[0]).slice(0, 2).join('')}
        </div>

        {/* Name + dept */}
        <div className="min-w-0 flex-1">
          <p className="text-sm font-semibold text-foreground truncate">{summary.employeeName}</p>
          <p className="text-xs text-foreground/50 truncate">{summary.staffId} · {summary.department}</p>
        </div>
        ...
        <div className="hidden lg:flex items-center gap-5 flex-shrink-0">
          <div className="text-center">
            <p className="text-xs font-bold text-foreground/70">{summary.totalWorking}</p>
            <p className="text-[10px] text-foreground/40">Rostered</p>
          </div>
          <div className="text-center">
            <p className="text-xs font-bold text-green-600">{summary.compliant}</p>
            <p className="text-[10px] text-foreground/40">On Time</p>
          </div>
          <div className="text-center">
            <p className={`text-xs font-bold ${summary.late > 0 ? 'text-amber-600' : 'text-foreground/30'}`}>{summary.late}</p>
            <p className="text-[10px] text-foreground/40">Late</p>
          </div>
          <div className="text-center">
            <p className={`text-xs font-bold ${summary.noShow > 0 ? 'text-red-600' : 'text-foreground/30'}`}>{summary.noShow}</p>
            <p className="text-[10px] text-foreground/40">No Show</p>
          </div>
          <div className="text-center">
            <p className="text-xs font-bold text-foreground/40">{summary.onLeave}</p>
            <p className="text-[10px] text-foreground/40">Leave</p>
          </div>
          {summary.totalLateMinutes > 0 && (
            <div className="text-center">
              <p className="text-xs font-bold text-amber-600">{summary.totalLateMinutes}m</p>
              <p className="text-[10px] text-foreground/40">Total Late</p>
            </div>
          )}
        </div>
        ...
        {/* Expand chevron */}
        <svg
          className={`w-4 h-4 text-foreground/40 flex-shrink-0 transition-transform ${expanded ? 'rotate-180' : ''}`}
...
export default function CompliancePage() {
  ...
  const rateColor = overall.rate >= 90 ? 'text-green-600' : overall.rate >= 75 ? 'text-amber-600' : 'text-red-600'
  ...
  return (
    <div className="p-6 max-w-[1400px] mx-auto space-y-5">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Shift Adherence</h1>
      ...
      <div className="flex flex-wrap gap-2">
        <button
          ...
          className={`px-3 py-1.5 rounded-full text-xs font-semibold border transition-all ${
            deptFilter === 'all' ? 'bg-foreground text-background border-foreground' : 'border-foreground/15 text-foreground/50 hover:text-foreground/70 hover:border-foreground/20'
          }`}
        >
          All Departments
        </button>
        {departments.map(d => (
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-xs font-semibold border transition-all ${
              deptFilter === d.id ? 'bg-foreground text-background border-foreground' : 'border-foreground/15 text-foreground/50 hover:text-foreground/70 hover:border-foreground/20'
            }`}
          >
            {d.name}
          </button>
      ...
      <div className="flex flex-wrap gap-2 items-center">
        <span className="text-xs text-foreground/40 mr-1">Legend:</span>
          ...
          {([
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all ${
                sortBy === v ? 'bg-foreground/8 text-foreground border-foreground/20' : 'border-foreground/10 text-foreground/50 hover:text-foreground/70'
              }`}
            >
              {l}
            </button>
      ...
      {/* Employee count */}
      {!loading && (
        <p className="text-xs text-foreground/40">{filtered.length} employee{filtered.length !== 1 ? 's' : ''} · {MONTHS[month-1]} {year}</p>
      )}

      {/* Employee list */}
      {loading ? (
        <div className="py-20 text-center text-foreground/40">
          <svg className="w-8 h-8 animate-spin mx-auto mb-3 text-foreground/30" fill="none" viewBox="0 0 24 24">
        ...
        <div className="py-20 text-center">
          ...
          <p className="text-sm font-semibold text-foreground/50">No roster or attendance data for this period</p>
          <p className="text-xs text-foreground/30 mt-1">Publish a roster and sync attendance to see compliance</p>
        </div>
      ) : filtered.length === 0 ? (
        <div className="py-12 text-center text-foreground/40 text-sm">No employees match your search</div>
      ) : (

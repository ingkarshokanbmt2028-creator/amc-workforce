...
export function ShiftModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-card border border-foreground/10 rounded-2xl shadow-2xl w-full max-w-sm mx-4 overflow-hidden">

        {/* Header */}
        <div className="px-5 pt-5 pb-4 border-b border-foreground/8">
          <div className="flex items-start justify-between gap-2">
            <div>
              <p className="font-bold text-foreground text-base leading-tight">Assign Shift</p>
              <p className="text-xs text-foreground/40 mt-1">
                <span className="text-foreground/70 font-medium">{employeeName}</span>
                {' · '}
                {new Date(date + 'T12:00:00').toLocaleDateString('en-GB', { weekday: 'short', day: 'numeric', month: 'short' })}
              </p>
            </div>
            <button onClick={onClose} className="text-foreground/30 hover:text-foreground/60 transition-colors p-1">
          ...
          {ruleLabel && (
            <div className="mt-2.5 flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-amber-500/10 border border-amber-500/20">
              ...
              <span className="text-[10px] text-amber-700">{ruleLabel}</span>
            </div>
          )}
        </div>
        ...
        <div className="px-5 py-4 space-y-4">
          {GROUPS.map(group => {
            ...
            return (
              <div key={group.label}>
                <p className="text-[10px] font-semibold uppercase tracking-widest text-foreground/40 mb-2">{group.label}</p>
                <div className="grid grid-cols-2 gap-1.5">
                  {groupShifts.map(shift => {
                    const times = SHIFT_TIMES[shift]
                    const hours = SHIFT_HOURS[shift]
                    const colorCls = SHIFT_COLORS[shift] ?? 'bg-slate-100 text-slate-600'
                    ...
                    return (
                      <button
                        ...
                        className={`relative text-left rounded-xl px-3 py-2.5 border transition-all ${
                          isSelected
                            ? 'ring-2 ring-foreground/20 border-foreground/20 bg-foreground/8'
                            : 'border-foreground/8 hover:border-foreground/15 hover:bg-foreground/5'
                        }`}
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <div className="min-w-0">
                            <p className="text-xs font-semibold text-foreground leading-tight truncate">{shift.replace(/_/g, ' ')}</p>
                            {times
                              ? <p className="text-[9px] text-foreground/40 leading-tight">{fmt12(times.start)} – {fmt12(times.end)}</p>
                              : <p className="text-[9px] text-foreground/40">—</p>
                            }
                          </div>
                        </div>
                        {hours != null && hours > 0 && (
                          <span className="absolute top-1.5 right-2 text-[9px] text-foreground/40 font-semibold">{hours}h</span>
                        )}
                        {isSelected && (
        ...
        <div className="px-5 pb-5 flex gap-3">
          <button
            onClick={onClose}
            className="flex-1 py-2.5 rounded-xl border border-foreground/10 text-sm text-foreground/50 hover:text-foreground/70 hover:bg-foreground/5 transition-colors"
          >
            Cancel
          </button>
          <button
            ...
            className="flex-1 py-2.5 rounded-xl bg-foreground text-background text-sm font-semibold hover:opacity-90 disabled:opacity-30 transition-all"
          >
